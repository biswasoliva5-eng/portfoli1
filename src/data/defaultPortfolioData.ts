import { PortfolioData, SiteSettings } from '../types.js';
import { getIdbPortfolioData, setIdbPortfolioData } from './idbStorage.js';
...
export const defaultPortfolioData: PortfolioData = {
  ...
  cv: {
    ...
    uploadedAt: '2026-09-19T10:09:19.385Z',
    sizeBytes: 184520
  },
  socialLinks: [
    {
...
const STORAGE_PORTFOLIO_KEY = 'oliva_portfolio_local_data';
const STORAGE_ADMIN_PASSWORD_KEY = 'oliva_admin_local_password';

let cachedLocalPortfolioData: PortfolioData | null = null;

// Background hydration from IndexedDB on initial import
if (typeof window !== 'undefined') {
  getIdbPortfolioData().then((idbData) => {
    if (idbData && idbData.settings && idbData.artworks) {
      cachedLocalPortfolioData = {
...
export function getLocalPortfolioData(): PortfolioData {
  if (cachedLocalPortfolioData) {
    return cachedLocalPortfolioData;
  }
  try {
    const raw = localStorage.getItem(STORAGE_PORTFOLIO_KEY);
    if (raw) {
...
export async function getLocalPortfolioDataAsync(): Promise<PortfolioData> {
  ...
  const idb = await getIdbPortfolioData();
  if (idb && idb.settings && idb.artworks) {
    const data: PortfolioData = {
      ...defaultPortfolioData,
      ...idb,
      settings: { ...defaultSettings, ...(idb.settings || {}) },
    };
    cachedLocalPortfolioData = data;
    return data;
  }
...
export function saveLocalPortfolioData(data: PortfolioData): void {
  cachedLocalPortfolioData = data;
  // Persist to IndexedDB immediately (handles large images & video easily without size limits)
  setIdbPortfolioData(data).catch(e => console.error('IndexedDB save error:', e));

  // Also attempt localStorage for quick sync, ignore quota error if data is too big
  try {
    localStorage.setItem(STORAGE_PORTFOLIO_KEY, JSON.stringify(data));
  } catch (e) {
    // QuotaExceededError is normal for large files; IndexedDB safely stored it!
    console.warn('localStorage quota exceeded; full data safely stored in IndexedDB.');
  }
}

export async function saveLocalPortfolioDataAsync(data: PortfolioData): Promise<void> {
  cachedLocalPortfolioData = data;
  await setIdbPortfolioData(data);
  try {
    localStorage.setItem(STORAGE_PORTFOLIO_KEY, JSON.stringify(data));
  } catch {
    // quota exceeded handled by idb
  }
}

export function getLocalAdminPassword(): string {
  try {
    return localStorage.getItem(STORAGE_ADMIN_PASSWORD_KEY) || 'oliva23';
  } catch {
    return 'oliva23';
  }
}

export function setLocalAdminPassword(newPass: string): void {
  try {
    localStorage.setItem(STORAGE_ADMIN_PASSWORD_KEY, newPass);
  } catch (e) {
    console.error('Error saving local admin password:', e);
  }
}
